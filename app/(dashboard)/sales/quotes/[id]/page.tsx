...
export default async function QuotationDetailPage({ params }: { params: any }) {
  ...
  const isExpired = new Date(quote.expiryDate || quote.createdAt) < new Date() && !isConverted;
    ...
    <div className="p-8 space-y-10 max-w-6xl mx-auto h-full overflow-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="border-outline-variant/30 text-[10px] uppercase font-black tracking-widest px-3 py-1 bg-surface-container-low">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="bg-surface/[0.03] border border-outline-variant/20 rounded-[40px] p-10 space-y-10 shadow-2xl overflow-hidden relative">
              <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 blur-[100px] -mr-32 -mt-32" />
              
              <div className="flex flex-col md:flex-row justify-between gap-10 relative z-10">
                <div className="space-y-4">
                  <div>
                    <h1 className="text-4xl font-black text-on-surface italic tracking-tighter">Commercial Proposal</h1>
                ...
                <div className="text-right space-y-2">
                   ...
                   <h2 className="text-6xl font-black text-on-surface italic tracking-tighter">${quote.totalAmount.toLocaleString()}</h2>
                </div>
              </div>

              {/* Items Table */}
              <div className="rounded-3xl border border-outline-variant/20 overflow-hidden bg-surface-container-low relative z-10">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-surface-container text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {quote.items.map((item: any) => (
                      <tr key={item.id} className="text-on-surface/80">
              ...
              <div className="space-y-3 relative z-10">
                ...
                <div className="p-6 bg-surface-container-low rounded-2xl border border-outline-variant/30 text-xs text-on-surface/60 leading-relaxed italic">
        ...
        <div className="space-y-6">
           ...
           <div className="bg-indigo-600 rounded-[32px] p-8 shadow-[0_20px_50px_rgba(79,70,229,0.3)] space-y-6 flex flex-col items-center text-center">
              ...
              <div className="space-y-1">
                 <h3 className="text-xl font-black text-on-surface uppercase italic tracking-tighter">Convert to Sale</h3>
              ...
              {!isConverted && !isExpired ? (
                <Link href={`/sales/new?fromQuote=${quote.id}`} className="w-full">
                  <Button className="w-full h-14 bg-surface text-indigo-600 hover:bg-surface/90 font-black uppercase tracking-widest rounded-2xl shadow-xl transition-all active:scale-95">
                    Accept & Finalize
                  </Button>
                </Link>
              ) : (
                <Button disabled className="w-full h-14 bg-surface-container-high text-on-surface-variant/50 font-black uppercase tracking-widest rounded-2xl">
           ...
           <div className="bg-surface/[0.03] border border-outline-variant/20 rounded-[32px] p-8 space-y-4">
              ...
              <Button variant="ghost" className="w-full h-12 justify-start gap-3 hover:bg-rose-500/10 hover:text-rose-500 rounded-xl font-bold uppercase text-[10px] tracking-widest text-on-surface/40 transition-colors">
             ...
             <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-[32px] p-8 space-y-4">
                ...
                {quote.salesInvoices.map((inv: any) => (
                   <Link key={inv.id} href={`/sales/${inv.id}`} className="flex justify-between items-center p-3 bg-emerald-500/5 rounded-xl border border-emerald-500/10 hover:bg-emerald-500/10 transition-colors">
                      <span className="text-[10px] font-black text-on-surface">{inv.invoiceNumber}</span>
...
function formatStatus(status: string) {
  switch (status) {
    ...
    case 'EXPIRED': return <Badge className="bg-surface-container text-muted-foreground border-outline-variant/30 text-[9px] font-black uppercase tracking-widest px-2 py-1">Expired</Badge>;
    default: return <Badge className="bg-surface-container-low text-on-surface/40 border-outline-variant/20 text-[9px] font-black uppercase tracking-widest px-2 py-1">Draft</Badge>;
  }
}
