...
export default async function PurchaseInvoiceDetailPage({ params }: { params: { id: string } }) {
  const invoice: any = await getPurchaseInvoiceById(params.id);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-outline-variant/20 pb-10">
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <h1 className="text-4xl font-black tracking-tighter text-on-surface">Manifest <span className="text-primary italic">{invoice.invoiceNumber}</span></h1>
              ...
              {invoice.items.map((item: any) => (
                <div key={item.id} className="flex items-center justify-between p-6 bg-slate-900/50 rounded-2xl border border-outline-variant/20">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-black text-on-surface">{item.quantity} x ${item.unitCost.toFixed(2)}</p>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-on-surface font-medium">{format(new Date(invoice.issueDate || invoice.createdAt), "MMMM dd, yyyy")}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-on-surface font-medium">{invoice.supplier.name}</p>
                </div>
              </div>
