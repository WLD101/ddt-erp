...
export default async function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-8 space-y-10 max-w-6xl mx-auto h-full overflow-auto">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-outline-variant/20 pb-10">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-6">
             <div className="h-20 w-20 rounded-[28px] bg-slate-900 border border-outline-variant/20 flex items-center justify-center shadow-2xl">
                {account.type === 'BANK' ? (
             ...
             <div className="space-y-1">
                <div className="flex items-center gap-3">
                  <h1 className="text-4xl font-black tracking-tighter text-on-surface italic">{account.name}</h1>
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="bg-surface-container-low border-outline-variant/30 rounded-xl h-12 px-6 text-on-surface font-black text-xs uppercase tracking-widest gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 bg-surface/[0.03] border border-outline-variant/20 rounded-[32px] p-8 space-y-6">
           <div className="space-y-1">
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest italic">Current Real-time Balance</p>
              <p className="text-5xl font-black text-on-surface italic tracking-tighter leading-none">${account.currentBalance.toLocaleString()}</p>
           </div>
           <div className="pt-6 border-t border-outline-variant/20 flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-lg font-bold text-on-surface">${account.currentBalance.toLocaleString()}</p>
              </div>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-surface/[0.03] border border-outline-variant/20 rounded-[32px] overflow-hidden">
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-white/5">
                {(account.ledgerEntries || []).map((entry: any) => (
                  <tr key={entry.id} className="group hover:bg-surface/[0.02] transition-colors">
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-on-surface tracking-widest uppercase">{format(new Date(entry.createdAt), "MMM dd")}</span>
                        <span className="text-[9px] text-muted-foreground">{format(new Date(entry.createdAt), "hh:mm a")}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="space-y-1">
                        <p className="text-xs font-bold text-on-surface leading-tight uppercase tracking-tight">{entry.description}</p>
                        <Badge variant="outline" className="text-[8px] h-4 font-black uppercase tracking-tighter bg-surface-container-low border-outline-variant/30 text-muted-foreground">
                          {entry.referenceType}
                        </Badge>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5 text-right whitespace-nowrap">
                      <p className="text-xs font-black text-on-surface italic opacity-60">
