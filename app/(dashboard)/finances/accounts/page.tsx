  ...
  return (
    <div className="p-8 space-y-10 max-w-7xl mx-auto h-full overflow-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Button className="h-12 px-6 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-black text-xs uppercase tracking-widest rounded-xl flex items-center gap-2 transition-all">
            <ArrowRightLeft className="w-4 h-4 text-primary" />
            Transfer Funds
          </Button>
          <Button className="h-12 px-6 bg-primary hover:bg-primary/90 text-white font-black text-xs uppercase tracking-widest rounded-xl flex items-center gap-2 transition-transform active:scale-95">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         <Card className="bg-white/5 border-white/5 shadow-xl glass-morphism rounded-3xl overflow-hidden relative group md:col-span-2">
         ...
         <Card className="bg-white/5 border-white/5 shadow-xl glass-morphism rounded-3xl overflow-hidden md:col-span-1">
           <CardHeader className="pb-2">
             <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accounts.map((account) => (
          <Link 
            ...
            className="group block bg-white/[0.03] border border-white/5 hover:border-primary/30 rounded-[32px] p-8 transition-all duration-300 relative overflow-hidden"
          >
            {/* Gloss Highlight */}
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex items-start justify-between">
                <div className="h-14 w-14 rounded-2xl bg-slate-900 border border-white/5 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                  {account.type === 'BANK' ? (
                ...
                <Badge variant="outline" className="bg-white/5 border-white/10 text-[10px] font-black tracking-widest uppercase py-1 px-3">
                  {account.type}
                </Badge>
              </div>

              <div className="space-y-1">
                <h4 className="text-xl font-black text-white tracking-tight group-hover:text-primary transition-colors">{account.name}</h4>
              ...
              <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                 <div className="space-y-0.5">
                    ...
                    <p className="text-2xl font-black text-white italic">${account.currentBalance.toLocaleString()}</p>
                 </div>
