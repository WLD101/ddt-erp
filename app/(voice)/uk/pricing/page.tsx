...
export const metadata: Metadata = {
  title: "WhatsQuery Voice UK Pricing",
  description: "Localized GBP pricing profile for WhatsQuery Voice in the United Kingdom.",
  alternates: { canonical: "/uk/pricing" },
};

export default async function UkPricingPage() {
  const host = await getVoiceRequestHost();
  const pricing = getPricingProfile("uk");
  const homeHref = toVoiceExternalPath("/uk", host);
  const loginHref = toVoiceExternalPath("/login", host);
  const onboardingHref = toVoiceExternalPath("/onboarding", host);
  const pricingHref = toVoiceExternalPath("/uk/pricing", host);
    ...
    <VoiceMarketingShell homeHref={homeHref} loginHref={loginHref} onboardingHref={onboardingHref} pricingHref={pricingHref} docsHref={docsHref}>
      <main className="mx-auto max-w-6xl px-6 py-14">
        <div className="rounded-[32px] border border-white/10 bg-slate-950/40 p-8 md:p-10">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-cyan-200">United Kingdom pricing</p>
          <h1 className="mt-3 text-4xl font-black text-white">GBP-first pricing for UK voice deployments.</h1>
